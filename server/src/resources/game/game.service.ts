...
export class GameService {
  ...
  generateBoard(
    size: number,
    totalMines: number,
  ): { board: string[][]; revealed: (boolean | null)[][] } {
    const revealed = Array.from({ length: size }, () => Array(size).fill(null));
    const board = Array.from({ length: size }, () =>
      Array(size).fill(GameStatus.SAVE),
    );
  ...
  async play(dto: NewPlayDto, payload: UserPayload) {
    ...
    if (game.revealed[dto.row][dto.col] !== null)
      throw new BadRequestException(
        'Cell already revealed or invalid position.',
      );
  ...
  async finish(dto: FinishDto, payload: UserPayload) {
    const session = await this.gameModel.db.startSession();
    session.startTransaction();
    ...
    try {
      ...
      const user = await this.userService.findOne(game.userId);
      if (!user) throw new BadRequestException('User not found!');

      game.isGameOver = true;
      game.finishedAt = new Date();
      game.winAmount = game.betAmount * game.lastOdd;
